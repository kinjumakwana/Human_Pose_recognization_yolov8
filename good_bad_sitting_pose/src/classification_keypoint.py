...
class KeypointClassification:
    def __init__(self, path_model):
        ...
        self.classes = ['Bad_Sitting_pose','Good_Sitting_pose']
...
if __name__ == '__main__':
    keypoint_classification = KeypointClassification(
        path_model='good_bad_sitting_pose/pose_classification.pt'
    )
    dummy_input = torch.randn(1,23)
    
    # Ensure that the input tensor is on the same device as the model
    dummy_input = dummy_input.to(keypoint_classification.device)
